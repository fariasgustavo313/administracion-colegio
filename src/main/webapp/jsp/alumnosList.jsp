...
<table class="table table table-striped-columns table-hover">
	...
	<%
		...
		for(Alumno alumno : alumnoRepository.getLikeApellido(buscar)) {
			curso = new CursoRepository(Connector.getConnection()).getById(alumno.getId_curso());
			out.println("<tr>");
			out.println("<th scope='row'>" + alumno.getId() + "</th>");
			out.println("<th scope='row'>" + alumno.getNombre() + "</th>");
			out.println("<th scope='row'>" + alumno.getApellido() + "</th>");
			out.println("<th scope='row'>" + alumno.getEdad() + "</th>");
			out.println("<th scope='row'>" + alumno.getId_curso() + "</th>");
			...
			out.println("<td >");
	%>
			<form action="jsp/alumnosRemove.jsp" onsubmit="return confirmarEliminacion()">
			<input type="text" name="idAlumno" value="<% out.println(alumno.getId()); %>" hidden=true>
